"""Document endpoints. HTTP only, no SQL."""
...
# One name for "a database session, injected by FastAPI".
SessionDep = Annotated[AsyncSession, Depends(get_session)]


@router.post("", response_model=DocumentRead)
async def create_document(
    body: DocumentCreate, response: Response, session: SessionDep
) -> DocumentRead:
    """Create the document, or update it only if the content changed."""
    document, changed = await repo.upsert(
        session, body.source, body.external_id, body.title, body.content
    )

    response.status_code = status.HTTP_201_CREATED if changed else status.HTTP_200_OK
    log.info("document_upserted", document_id=str(document.id), changed=changed)

    return DocumentRead.model_validate(document)


@router.get("/{document_id}", response_model=DocumentRead)
async def read_document(document_id: uuid.UUID, session: SessionDep) -> DocumentRead:
    """One document by id."""
